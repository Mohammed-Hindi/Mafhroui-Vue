  ...
  <div>
    <div class="flex flex-wrap items-center justify-between gap-4 mb-6">
      <div class="flex items-center gap-3">
        ...
        <div>
          <h3 class="text-h3 font-bold text-text-900">نسبة تقدّم فرقي</h3>
          <p class="text-caption text-text-600">متابعة تقدّم فرقك وتصفية النتائج حسب الحالة والفريق</p>
        </div>
      </div>
    ...
    >
      ...
      <template #cell-proj="{ row }">
        ...
        <router-link :to="{ name: 'supervisor-teams' }" class="flex items-center gap-1.5 text-label text-primary-700 hover:text-primary-800 hover:underline transition-colors duration-fast mt-0.5">
    ...
    rows() {
      return this.overview.map((entry) => {
        ...
        return {
          ...
          team: team.name,
          dept: team.project?.department?.name || 'غير محدد',
    ...
    async exportExcel() {
      ...
      try {
        const rowGroups = this.filteredRows.map((r) => [{
          team: r.team, dept: r.dept, spec: r.spec,
          status: STATUS_LABELS[r.status]?.label || r.status,
          done: r.done, total: r.total, pct: `${r.pct}%`
        }])
        await exportStyledExcel({
          fileName: 'نسبة-تقدم-فرقي.xlsx',
          sheetTitle: 'نسبة تقدّم فرقي',
          columns: [
            { key: 'team', label: 'الفريق', width: 24 },
            { key: 'dept', label: 'القسم', width: 18 },
